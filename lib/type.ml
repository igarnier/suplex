module type S = sig
  ...
  type ex_repr = Ex_repr : 'a repr -> ex_repr
    ...
    fun (type a b) (x : a typ) (y : b typ) ->
     ...
     | (TVec tx, TVec ty) ->
         (match type_eq tx ty with None -> None | Some Refl_eq -> Some Refl_eq
           : (a, b) eq option)
     | (TTpl l1, TTpl l2) ->
         if ex_type_list_eq l1 l2 then Some Refl_eq else None
  ...
  let rec show_typ : 'a. 'a typ -> string =
    fun (type a) (typ : a typ) ->
     ...
     | TVec t ->
         ...
         Printf.sprintf "<%s>" s
     | TTpl typs ->
         let s = List.map (function Ex_typ t -> show_typ t) typs in
         String.concat "," s |> Printf.sprintf "(%s)"
     | TFun { arg; ret } ->
