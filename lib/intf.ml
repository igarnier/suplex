...
module type Cast = sig
  ...
  (** [trunc n1 n2] constructs a truncating cast from [n1] to [n2].
      Returns [None] if a truncation doesn't exist, e.g. if [n1] is
      smaller than [n2]. *)
...
module type Stack_frame = sig
  ...
  (** [arr ty len] describes a stack element storing an array of [ty] with length [len]. *)
  val arr : 'a typ -> i64 m -> ('a, [ `unk ]) arr stack_var

  (** [arr_cst ty len] describes a stack element storing an array of [ty] with statically known length [len]. *)
...
module type Monad = sig
  ...
  (** [cfg] is the type of parameters for running an effectful computation generating
      code. Typically, this will correspond to parameters of the code generator. *)
  type cfg
...
(** [S] is the main module type provided by {b suplex}. It exposes a
    low-level, C-like language. Users describe programs in this language
    and {b suplex} provides facilities to run the code from OCaml. *)
module type S = sig
  type +!'a k
  ...
  (** The [Stack] module is used to construct stack descriptors for function declarations. *)
  module Stack :
    Stack_frame
  ...
  (** [register_external ~name proto] returns a function with name [name]
      and prototype [proto], assuming that a [C] function with the corresponding
  ...
  *)
  ...
  (** [string ?z str] constructs a constant string with contents [str].
      If [z] is set to false, the resulting string is not zero-terminated. *)
  ...
  (** [projs r] constructs the projection functions out of a record description [r]. *)
  val projs : ('elim, 'a vec, 'a vec, 'u) Types.record -> 'elim

  (** [strct.%{field}] evaluates the projection [field] on the structure [strct].

      Note that the structure is accessed through a pointer. Accessing a
      badly or null pointer is undefined behaviour.  *)
  ...
  (** [seq m @@ fun () -> rest] corresponds to evaluating [m] sequentially then evaluating [rest].
      This is equivalent to [let*! _ = m in rest]. *)
  val seq : unit m -> (unit -> 'a m) -> 'a m

  (** [let*! x = m in rest] performs code generation for [m], binds the result to [x] and
      perform code generation for [m].

      Note that this is not equivalent to simply substituting [x] for [m] in [rest]. *)
  ...
  (** [set arr i v] sets the value [v] at index [i] in [arr].
      No bound checking is performed. *)
  val set : ('a, 'c) arr m -> i64 m -> 'a m -> unit m

  (** [setaddr arr i v] sets the address of the first element of [v] at index [i] in [arr].
      No bound checking is performed. *)
  val setaddr :
  ...
  (** [for_ ~init ~pred ~step body] generates a for loop. This is implements the C-like
      form
      {[
  ...
  *)
  ...
  (** [foldi_ ~init ~acc ~pred ~step body] generates a fold over the sequence
      of integers defined by [init], [pred] and [step].
  *)
  val foldi :
    init:i64 m ->
    acc:'a m ->
  ...
  (** [global_array num arr] creates a global array containing elements of the numerical type
      [num]. *)
  val global_array :
  ...
  (** [malloc ty] allocates a value of type [ty] on the heap and returns a pointer to that value. *)
  val malloc : 'a typ -> 'a ptr m

  (** [malloc_array ty numel] allocates an array with elements of type [ty] and [numel] elements
      on the heap. *)
  val malloc_array : 'a typ -> i64 m -> ('a, [ `unk ]) arr m

  (** [free ptr] frees some heap-allocated memory. Freeing an invalid pointer
  ...
  (** [fundecl name prototype body] defines a function [name] with arguments
      described by [prototype]. [body] takes as
      argument pointers to the data on the stack followed by the arguments described in
      the prototype. *)
  val fundecl :
  ...
  module Exec : sig
    ...
    (** [('s, 'o) rel] is the type of witnesses that {b suplex} expressions of
        ...
        This type is used to describe how one feeds data to a {b suplex}-generated
        function. *)
    type (_, _) rel
    ...
    (** ['a opaque] is used to hide the actual details of a {b suplex} record type
        on the OCaml side. *)
    type _ opaque
    ...
    (** [vec |+ rel] extends the relation vector [vec] with
        [rel]. Note that this matches the syntax of record construction. *)
    val ( |+ ) :
      ('c Vec.t, 'd Vec.t) rel_vec ->
      ('a m, 'b) rel ->
      (('a * 'c) Vec.t, ('b * 'd) Vec.t) rel_vec

    (** [BA] is the module type of relations between {b suplex} arrays
        and bigarrays. *)
    module type BA = sig
      (** The type of elements in the array on the {b suplex} side. *)
      type elt

      (** The type of suplex 'big arrays' (see the record definition {!r} below). *)
      type s

      (** In {b suplex}, bigarrays are mapped to two-elements records
          that contain the length of the bigarray and a pointer to the data.

          The record definition [r] corresponds to the definition of this
          record type. *)
      val r :
      ...
      (** [dim] is a projection used to get the number of elements
          from a {b suplex} 'big array' *)
      val dim : (i64, s) proj
    end

    (** [I64_ba] is the module allowing to access a bigarray containing 64-bit signed integers. *)
    module I64_ba : BA with type elt = i64

    (** [I32_ba] is the module allowing to access a bigarray containing 32-bit signed integers. *)
    module I32_ba : BA with type elt = i32

    (** [I16_ba] is the module allowing to access a bigarray containing 16-bit signed integers. *)
    module I16_ba : BA with type elt = i16

    (** [I8_ba] is the module allowing to access a bigarray containing 8-bit signed integers. *)
    module I8_ba : BA with type elt = i8

    (** [F64_ba] is the module allowing to access a bigarray containing double-precision floating point numbers. *)
    module F64_ba : BA with type elt = f64

    (** [F32_ba] is the module allowing to access a bigarray containing double-precision floating point numbers. *)
    module F32_ba : BA with type elt = f32

    (** [bigarray_i64] witnesses that [int64] bigarrays can mapped to [I64_ba.s]. *)
    val bigarray_i64 :
      ( I64_ba.s ptr m,
        (int64, Bigarray.int64_elt, Bigarray.c_layout) Bigarray.Array1.t )
      rel

    (** [bigarray_i32] witnesses that [int32] bigarrays can mapped to [I32_ba.s]. *)
    val bigarray_i32 :
      ( I32_ba.s ptr m,
        (int32, Bigarray.int32_elt, Bigarray.c_layout) Bigarray.Array1.t )
      rel

    (** [bigarray_i16] witnesses that [int16] bigarrays can mapped to [I16_ba.s]. *)
    val bigarray_i16 :
      ( I16_ba.s ptr m,
    ...
    (** [bigarray_f64] witnesses that [float64] bigarrays can mapped to [F64_ba.s]. *)
    val bigarray_f64 :
      ( F64_ba.s ptr m,
        (float, Bigarray.float64_elt, Bigarray.c_layout) Bigarray.Array1.t )
      rel

    (** [bigarray_f32] witnesses that [float32] bigarrays can mapped to [F32_ba.s]. *)
    val bigarray_f32 :
      ( F32_ba.s ptr m,
    ...
    (** [cfg] is the type of the configuration used by the
        underlying code generation (e.g. LLVM). *)
    type cfg
    ...
    (** [add_fdecl f frel mdl] extends the module [mdl] with a function declaration
        [f] with OCaml type defined by [frel]. *)
    val add_fdecl :
      's fundecl ->
      ('s, 'dom -> 'range) fn_rel ->
      'r module_ ->
    ...
    (** [run_module ?cfg mdl] evaluates [mdl] and returns the
        evaluated module. *)
    ...
    (** [run ?cfg ?fname rel body] is as [run_program] but automatically
        derives the prototype from [rel]. *)
    val run :
      ?cfg:cfg ->
      ?fname:string ->
      ('f, 'b -> 'c) fn_rel ->
