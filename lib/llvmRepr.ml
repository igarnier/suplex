...
struct
  ...
  let return result state =
    { state ; result }
  let llreturn llval typewit state =
    { state ; result = { llval ; typewit } }
...
and T : Type.S with type 'a repr = 'a State.llvm State.t = 
  Type.Inst(struct type nonrec 'a repr = 'a State.llvm State.t end)
...
and tuple_type_to_llvm_struct (tuple : T.ex_typ list) : Llvm.lltype State.t = 
  ...
  let rec loop (tuple : T.ex_typ list) acc =
    ...
    | (T.Ex_typ field_type) :: tl ->
      (type_to_llvm field_type) >>= fun typ ->
      loop tl (typ :: acc)
  in
  context >>= fun ctxt ->
  loop tuple [] >>= fun res ->
...
struct
  ...
  let all =
    ...
    [
      { name = Print_int64 ;
        typ  = Ex_funtype { arg = TNum Int64 ;
                            ret = TUnit } } ;
      { name = Print_int32 ;
        typ  = Ex_funtype { arg = TNum Int32 ;
                            ret = TUnit } } ;
      { name = Print_int16 ;
        typ  = Ex_funtype { arg = TNum Int16 ;
                            ret = TUnit } };
      { name = Print_int8 ;
        typ  = Ex_funtype { arg = TNum Int8 ;
                            ret = TUnit } };
      { name = Print_float ;
        typ  = Ex_funtype { arg = TNum Float ;
                            ret = TUnit } };
      { name = Print_double ;
        typ  = Ex_funtype { arg = TNum Double ;
                            ret = TUnit } };
      { name = Print_bool ;
        typ  = Ex_funtype { arg = TBool ;
                            ret = TUnit } };
      { name = Print_unit ;
        typ  = Ex_funtype { arg = TUnit ;
                            ret = TUnit } };
      { name = Instralloc ;
        typ  = Ex_funtype { arg = TNum Int64 ;
                            ret = TPtr (TNum Int8) } }
...
let build_malloc_fptr lltype count =
  ...
  | Some malloc ->
    ...
    let llvalue   = Llvm.build_bitcast ptr (Llvm.pointer_type lltype) "alloc_ptr" bld in
    return llvalue
  | None ->
    failwith "Codegen.build_malloc_fptr: instralloc not declared"
...
let build_malloc_immediate lltype count =
  ...
  match Llvm.lookup_function "instralloc" llmodule with
  | Some malloc ->
    let sizeof   = Llvm.const_intcast (Llvm.size_of lltype) (size_t ctxt) ~is_signed:false in
    let bytesize = Llvm.build_mul count sizeof "multmp" bld in
    let downcast = Llvm.build_trunc bytesize (size_t ctxt) "casttmp" bld in
    let ptr      = Llvm.build_call malloc [| downcast |] "calltmp" bld in
    let llvalue  = Llvm.build_bitcast ptr (Llvm.pointer_type lltype) "alloc_ptr" bld in
    return llvalue
  | None ->
    failwith "Codegen.build_malloc: instralloc not declared"

let build_malloc (type a) (count : Llvm.llvalue) (typ : a T.typ) : Llvm.llvalue State.t =
  let open State in
...
let unit =
  let open State in
  context >>= fun ctx ->
  llreturn (Llvm.const_int (int8_t ctx) 0) (T.TUnit)
...
let int8 (i : int) =
  let open State in
  context >>= fun ctx ->
  llreturn (Llvm.const_int (int8_t ctx) i) T.(TNum Int8)
...
let tt =
  let open State in
  context >>= fun ctx ->
  llreturn (Llvm.const_int (bool_t ctx) 1) T.TBool

let ff =
  let open State in
  context >>= fun ctx ->
  llreturn (Llvm.const_int (bool_t ctx) 0) T.TBool
...
let add (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
    ...
    let llval =
      ...
      | `int ->
        Llvm.build_add lhs_v.llval rhs_v.llval "int_add_tmp" bld
      | `fp ->
        Llvm.build_fadd lhs_v.llval rhs_v.llval "float_add_tmp" bld
    in
...
let sub (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
  ...
  let llval =
    match T.numerical_kind numtyp with
    | `int ->
      Llvm.build_sub lhs_v.llval rhs_v.llval "int_sub_tmp" bld
    | `fp ->
      Llvm.build_fsub lhs_v.llval rhs_v.llval "float_sub_tmp" bld
  in
  llreturn llval T.(TNum numtyp)
...
let mul (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
  ...
  let llval =
    match T.numerical_kind numtyp with
    | `int ->
      Llvm.build_mul lhs_v.llval rhs_v.llval "int_mul_tmp" bld
    | `fp ->
      Llvm.build_fmul lhs_v.llval rhs_v.llval "float_mul_tmp" bld
  in
  llreturn llval T.(TNum numtyp)
...
let div (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
  ...
  let llval =
    match T.numerical_kind numtyp with
    | `int ->
      Llvm.build_sdiv lhs_v.llval rhs_v.llval "int_sdiv_tmp" bld
    | `fp ->
      Llvm.build_fdiv lhs_v.llval rhs_v.llval "float_div_tmp" bld
  in
  llreturn llval T.(TNum numtyp)
...
let lt (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
  ...
  let llval =
    ...
    | `fp ->
      Llvm.build_fcmp Llvm.Fcmp.Olt lhs_v.llval rhs_v.llval "float_lt_tmp" bld in
  llreturn llval T.TBool
...
let le (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
  ...
  let llval =
    ...
    | `fp ->
      Llvm.build_fcmp Llvm.Fcmp.Ole lhs_v.llval rhs_v.llval "float_le_tmp" bld in
  llreturn llval T.TBool
...
let eq (type t) (numtyp : t T.numerical) (lhs : t repr) (rhs : t repr) =
  ...
  let llval =
    ...
    | `fp ->
      Llvm.build_fcmp Llvm.Fcmp.Oeq lhs_v.llval rhs_v.llval "float_eq_tmp" bld in
  llreturn llval T.TBool
...
let print (type t) (arg : t repr) =
  ...
  let func =
    ...
    | T.TNum T.Int64 ->
      get_external ~name:Externals.Print_int64
    | T.TNum T.Int32 ->
      get_external ~name:Externals.Print_int32
    | T.TNum T.Int16 ->
      get_external ~name:Externals.Print_int16
    | T.TNum T.Int8 ->
      get_external ~name:Externals.Print_int8
    | T.TNum T.Float ->
      get_external ~name:Externals.Print_float
    | T.TNum T.Double ->
      get_external ~name:Externals.Print_double
    | T.TBool ->
      get_external ~name:Externals.Print_bool
    | T.TUnit ->
      get_external ~name:Externals.Print_unit
    | _ ->
      failwith "LlvmRepr.print: cannot print pointer, vector or functional values"
  in
  func >>= fun func_v ->
  builder >>= fun bld ->
...
let rec evaluate_tuple l acc =
  ...
  | (T.Ex_repr repr) :: tail ->
    repr >>= fun repr_v ->
    evaluate_tuple tail ((Ex_result repr_v) :: acc)
                       
let tuple (tuple : T.ex_repr list) =
  let open State in
  evaluate_tuple tuple [] >>= fun components ->
  let types = List.map components ~f:(function Ex_result repr_v -> T.(Ex_typ (repr_v.typewit))) in
...
let proj (type a) tuple ({ index ; desired_type } : a T.access) =
    ...
    | T.TTpl typ ->
      ...
      (match indexed_typ with
       | None ->
         failwith "LlvmRepr.proj: index out of bounds"
       | Some (T.Ex_typ actual_type) ->
         (match T.type_eq desired_type actual_type with
          | None ->
            ...
            let msg = 
              Printf.sprintf "LlvmRepr.proj: type mismatch, expected: %s vs actual: %s"
                desired_s 
                actual_s in
            failwith msg
          | Some _ ->
            let elt_ptr =
              Llvm.build_struct_gep tuple_v.llval index ("fieldaddr_"^(string_of_int index)) bld
            in
            let llvalue = Llvm.build_load elt_ptr "proj_load" bld in
            llreturn llvalue desired_type))

let (>|=) (type t u) (m : t repr) (f : t repr -> u repr) =
  fun state ->
    let m_v = m state in
    f (fun _ -> m_v) state
